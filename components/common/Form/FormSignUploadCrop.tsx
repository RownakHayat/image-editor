...
const FormSignUploadCrop = (props: Props) => {
  const {
    register,
    setValue,
    getValues,
  ...
  const [profileSignDelete] = useDeleteUserSignImageMutation()
  ...
  const handleDeleteSign = async () => {
    ...
    try {
      await profileSignDelete(props.userId);
...
export default FormSignUploadCrop;
